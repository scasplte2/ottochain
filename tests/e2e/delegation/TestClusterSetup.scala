  ...
  class TestCluster(
    ...
    def submitDelegation(
      ...
      for {
        ...
        result <- if (isValid) {
          ...
          activeDelegations.update(_ + (session.delegationId -> session)).as(
            DelegationResult(session.delegationId, isValid = true)
          )
        } else {
          IO.pure(DelegationResult(
    ...
    def submitDelegatedTransaction(
      ...
      for {
        ...
        result <- if (revokedSet.contains(delegationSession.delegationId)) {
          // Delegation is revoked
          IO.pure(TransactionResult(
          ...
          if (currentTime > activeDelegation.expiresAt) {
            ...
            for {
              ...
              result <- if (totalSpent + transactionAmount > activeDelegation.maxSpendLimit) {
                IO.pure(TransactionResult(
                  transactionHash = UUID.randomUUID().toString,
                  isAccepted = false,
                  isRejected = true,
    ...
    def waitForTransactionConfirmation(
      ...
      for {
        ...
        result <- history.find(_.transactionHash == transactionHash) match {
          case Some(tx) => IO.pure(TransactionState(
          ...
          case None => IO.pure(TransactionState(
            transactionHash = transactionHash,
            isConfirmed = false
          ))
        }
