...
object AuditRendererSuite extends SimpleIOSuite {
  ...
  test("renders each log entry with its fingerprint and the key facts") {
    ...
    IO.pure(
      ...
      expect(AuditRenderer.render(creation).contains("bound to escrow.package@1.0.0")) and
      expect(AuditRenderer.render(upgrade).contains("upgraded escrow.package@1.0.0 -> escrow.package@2.0.0")) and
