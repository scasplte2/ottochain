...
object Updates {
  ...
  object OttochainMessage {
    ...
    /**
     * TOTAL ordering for Signed[OttochainMessage]. The message-level `ordering` above is the PRIMARY key
     * (creates before transitions; per-fiber by sequence number) but is only PARTIAL -- same-name registry
     * ops and duplicate (fiberId, sequence) ops produce equal message keys. Complete it to a TOTAL order by
     * tiebreaking on the signature(s): they are part of the signed update (identical on every node), are made
     * over the canonical message bytes, and therefore distinguish any two distinct updates. This is pure (no
     * Hasher needed), so the combiner can simply `batch.sorted(signedOrdering)` and every node folds the
     * identical sequence -- the surviving op on a tie is deterministic; the loser is recorded as a
     * RejectionReceipt rather than aborting the batch.
     */
