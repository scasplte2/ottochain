...
|---|---|---|---|
| 1. The chart | private state, privately updated; replay denied | `zk-jlvm-shielded` (M5) + `groth16_verify` + nullifier map + `pmt_verify` root advance | `shieldApp` (sdk PR #212), corrected idioms |
...
| 4. One fill | one-time action, actor-cohort hidden | `sigma_verify` CDS OR-of-dhtuple + spent-nullifier map | `sigma-mixer` lane |
| 5. Audit | record provable vs consensus-signed root | `mpt_verify` client-side (`assertStateProof` runner action) | committed-state PR #164 |
...
arrays, a **map** + `has`/`set` for the nullifier spent-set, and `cat("0x", substr(…))`
re-prefixing for every public-values word. Additionally — an improvement over shield-app —
root advancement is not blind: the guard runs
...
opcode or bridge attestation); (c) no protocol nullifier set (privacy-handoff P0.1) — fiber
stateData is the stand-in; (d) time arithmetic lives inside proofs over private timestamps,
...
1. Protocol nullifier set with bounded growth (P0.1, `asset-shielded-mode.md`).
